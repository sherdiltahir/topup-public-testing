...
class DatabaseService {
  ...
  Stream<PetrolPump> getSelectedPetrolPump(String id){
    return petrolPumpCollection.document(id).snapshots().map(_petrolPumpDataFromSnapshot);
  }
  ...
  Stream<Operator> getSelectedOperator(String id){
    return operatorsCollection.document(id).snapshots().map(_operatorDataFromSnapshot);
  }
  ...
  Stream<List<Complain>> getSelectedUserComplains(String id){
    return complainsCollection.where('complainantId',isEqualTo: id).where('complainantId',isEqualTo: 'User').snapshots().map(_complainDataListFromSnapshot);
  }
  ...
  Future<void> setComplainData(Complain complain) async {
    return await complainsCollection.document(complain.complainId).setData(complain.toMap());
  }
  ...
  Stream<List<CouponsAvailable>> getAllAvailableCoupons(){
    return availableCouponsCollection.snapshots().map(_couponAvailableDataListFromSnapshot);
  }
  List<CouponsAvailable> _couponAvailableDataListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.documents.map((doc) {
  ...
  Stream<List<Coupon>> getAllCouponsForUser(String id){
    return couponsCollection.where("currentOwnerId",isEqualTo: id).snapshots().map(_couponDataListFromSnapshot);
  }
  ...
  Future<void> setCouponData(Coupon coupon) async {
    return await couponsCollection.document(coupon.couponId).setData(coupon.toMap());
  }
  ...
  Stream<List<CouponSharing>> getAllCouponTransactionsSentByUser(String id){
    return couponSharingCollection.where("sentBy",isEqualTo: id).snapshots().map(_couponSharingDataListFromSnapshot);
  }
  Stream<List<CouponSharing>> getAllCouponTransactionsReceivedByUser(String id){
    return couponSharingCollection.where("sentTo",isEqualTo: id).snapshots().map(_couponSharingDataListFromSnapshot);
  }
  List<CouponSharing> _couponSharingDataListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.documents.map((doc) {
  ...
  Future<void> setCouponSharingData(CouponSharing coupon) async {
    return await couponSharingCollection.document(coupon.couponId).setData(coupon.toMap());
  }
  ...
  Stream<List<FuelTransaction>> getAllFuelTransactionsForUser(String id){
    return fuelTransactionCollection.where("userId",isEqualTo: id).snapshots().map(_fuelTransactionDataListFromSnapshot);
  }
  Stream<FuelTransaction> getParticularFuelTransaction(String id){
    return fuelTransactionCollection.document(id).snapshots().map(_fuelTransactionDataFromSnapshot);
  }
  List<FuelTransaction> _fuelTransactionDataListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.documents.map((doc) {
  ...
  Future<void> setFuelTransactionData(FuelTransaction transaction) async {
    return await fuelTransactionCollection.document(transaction.transactionId).setData(transaction.toMap());
  }
  ...
  Stream<List<CouponsTransaction>> getAllCouponTransactionsForUser(String id){
    return couponTransactionCollection.where("userId",isEqualTo: id).snapshots().map(_couponTransactionDataListFromSnapshot);
  }
  Stream<CouponsTransaction> getParticularCouponTransaction(String id){
    return couponTransactionCollection.document(id).snapshots().map(_couponTransactionDataFromSnapshot);
  }
  List<CouponsTransaction> _couponTransactionDataListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.documents.map((doc) {
  ...
  Future<void> setCouponTransactionData(CouponsTransaction transaction) async {
    return await couponTransactionCollection.document(transaction.transactionId).setData(transaction.toMap());
  }
